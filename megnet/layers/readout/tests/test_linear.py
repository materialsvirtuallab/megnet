...
class TestLayer(unittest.TestCase):
    ...
    def test_linear(self):
        ...
        expected_output = np.concatenate(
            [np.mean(x[0, :3, :], axis=0, keepdims=True), np.mean(x[0, 3:, :], axis=0, keepdims=True)], axis=0
        )
        ...
        x = np.random.normal(size=(1, 5, 6))
        expected_output = np.concatenate(
            [np.sum(x[0, :3, :], axis=0, keepdims=True), np.sum(x[0, 3:, :], axis=0, keepdims=True)], axis=0
        )
        index = np.array([[0, 0, 0, 1, 1]])
        result = model.predict([x, index])
        diff = np.linalg.norm(result[0, :, :] - expected_output)
